$(function() {
	$.ajax({
				...
				data : {
					type : "课程评价",
					target : "getsurveyforsurveycenternew"
				},
				success : function(result, response, status) {
					var result = eval("(" + result + ")");
					$.each(result.data, function(index, item) {
								var showHtml = '<div class="row info-content">' + '<a href="test.html?' + item.id
										+ '" target=\'_blank\'><div class="col-md-5 col-sm-5 col-xs-5">'
										+ '<img src="../cover/' + item.image + '" class="img-responsive" alt="">'
										+ '</div>' + '<div class="col-md-7 col-sm-7 col-xs-7">' + '<h4>' + item.title
										+ '</h4>' + '<p class="hidden-xs">' + item.description + '</p>' + '<p>'
										+ item.createtime + '</p>' + '</div></a>' + '</div>'
								$(showHtml).appendTo("#kcpj div.container-fluid-new");
							});
				}
			});
	$.ajax({
		...
		data : {
			type : "课程评价",
			target : "getsurveyforsurveycenterhot"
		},
		success : function(result, response, status) {
			var result = eval("(" + result + ")");
			$.each(result.data, function(index, item) {
				var showHtml = '<div class="row">'
						+ '<a href="test.html?'
						+ item.id
						+ '" target=\'_blank\'><div class="col-md-5 col-sm-5 col-xs-5" style="margin: 12px 0; padding: 0">'
						+ '<img src="../cover/' + item.image + '"class="img-responsive" alt="">' + '</div>'
						+ '<div class="col-md-7 col-sm-7 col-xs-7" style="padding-right: 0">' + '<h4>' + item.title
						+ '</h4>' + '<p>' + item.createtime + '</p>' + '</div></a>' + '</div>'
				$(showHtml).appendTo("#kcpj div.container-fluid-hot");
			});
		}
	});
	$.ajax({
				...
				success : function(result, response, status) {
					...
					$.each(result.data, function(index, item) {
								var showHtml = '<div class="row info-content">' + '<a href="test.html?' + item.id
										...
										+ '</h4>' + '<p class="hidden-xs">' + item.description + '</p>' + '<p>'
										+ item.createtime + '</p>' + '</div></a>' + '</div>'
								$(showHtml).appendTo("#jxpj div.container-fluid-new");
							});
				}
			});
	$.ajax({
		...
		success : function(result, response, status) {
			...
			$.each(result.data, function(index, item) {
				var showHtml = '<div class="row">'
						...
						+ '<div class="col-md-7 col-sm-7 col-xs-7" style="padding-right: 0">' + '<h4>' + item.title
						+ '</h4>' + '<p>' + item.createtime + '</p>' + '</div></a>' + '</div>'
				$(showHtml).appendTo("#jxpj div.container-fluid-hot");
			});
		}
	});
	$.ajax({
				...
				success : function(result, response, status) {
					...
					$.each(result.data, function(index, item) {
								var showHtml = '<div class="row info-content">' + '<a href="test.html?' + item.id
										...
										+ '</h4>' + '<p class="hidden-xs">' + item.description + '</p>' + '<p>'
										+ item.createtime + '</p>' + '</div></a>' + '</div>'
								$(showHtml).appendTo("#jyqk div.container-fluid-new");
							});
				}
			});
	$.ajax({
		...
		success : function(result, response, status) {
			...
			$.each(result.data, function(index, item) {
				var showHtml = '<div class="row">'
						...
						+ '<div class="col-md-7 col-sm-7 col-xs-7" style="padding-right: 0">' + '<h4>' + item.title
						+ '</h4>' + '<p>' + item.createtime + '</p>' + '</div></a>' + '</div>'
				$(showHtml).appendTo("#jyqk div.container-fluid-hot");
			});
		}
	});
	$.ajax({
				...
				success : function(result, response, status) {
					...
					$.each(result.data, function(index, item) {
								var showHtml = '<div class="row info-content">' + '<a href="test.html?' + item.id
										...
										+ '</h4>' + '<p class="hidden-xs">' + item.description + '</p>' + '<p>'
										+ item.createtime + '</p>' + '</div></a>' + '</div>'
								$(showHtml).appendTo("#qtfm div.container-fluid-new");
							});
				}
			});
	$.ajax({
		...
		success : function(result, response, status) {
			...
			$.each(result.data, function(index, item) {
				var showHtml = '<div class="row">'
						...
						+ '<div class="col-md-7 col-sm-7 col-xs-7" style="padding-right: 0">' + '<h4>' + item.title
						+ '</h4>' + '<p>' + item.createtime + '</p>' + '</div></a>' + '</div>'
				$(showHtml).appendTo("#qtfm div.container-fluid-hot");
			});
		}
	});
	$.ajax({
	...
	$("div.tab-content").delegate("a", "click", function(e) {
	...
	$("#btn-search-survey").click(function() {
		var keyword = $("#input-search-survey").val();
		if (keyword == "") {
			s = "<span class='text-danger'><i class='ace-icon fa fa-times red'></i>请填写关键词！</span>";
			className = "btn-sm btn-danger";
			bootbox.dialog({
						message : "<h4 style='font-family: 微软雅黑'>" + s + "</h4>",
						buttons : {
			...
			$.ajax({
						...
						data : {
							kword : keyword,
							target : "getsurveyforsurveycentersearch"
						},
						success : function(result, response, status) {
							$("#searchresult div.modal-body-searchresult").empty();
							var result = eval("(" + result + ")");
							$.each(result.data, function(index, item) {
										var showHtml = '<div class="row info-content">' + '<a href="test.html?'
												+ item.id
												+ '" target=\'_blank\'><div class="col-md-5 col-sm-5 col-xs-5">'
												+ '<img src="../cover/' + item.image
												+ '" class="img-responsive" alt="">' + '</div>'
												+ '<div class="col-md-7 col-sm-7 col-xs-7">' + '<h4>' + item.title
												+ '</h4>' + '<p class="hidden-xs">' + item.description + '</p>' + '<p>'
												+ item.createtime + '</p>' + '</div></a>' + '</div>'
										$(showHtml).appendTo("#searchresult div.modal-body-searchresult");
									});
							$('#searchresult').modal();
						}
					});
