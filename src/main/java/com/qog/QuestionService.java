...
public class QuestionService implements Service {
	...
	public int updateAndReturnID(String title, String description, String image, String type, int userid, int id) {
		String strSql;
		if ("".equals(image)) {
			strSql = "UPDATE survey SET title=?,description=?,type=?,userid=?,createtime=CONVERT(varchar(20),GETDATE()) WHERE id=?";
			...
			strSql = "UPDATE survey SET title=?,description=?,image=?,type=?,userid=?,createtime=CONVERT(varchar(20),GETDATE()) WHERE id=?";
	 ...
	 * 获取指定surveyID的问题记录
	 */
	List<Map<String, Object>> getQuestionsBySurveyId(int surveyid) {
	 ...
	 * 获取指定surveyID的分析数据
	 */
	List<Map<String, Object>> getAnalysisBySurveyId(int surveyid) {
	 ...
	 */
	Map<String, Object> getSingleAnalysisBySurveyId(int surveyid, int number, String graduation) {
	 ...
	 */
	List<String> getAnalysisOptionsBySurveyId(int surveyid) {
				...
				if (!content.equals("")) {
					...
					if (opHitName.length > 0) {
						for (String string : opHitName) {
							...
							int affectrow = jdao.update(new PreparedStatementCreator() {
								public java.sql.PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
									int i = 0;
