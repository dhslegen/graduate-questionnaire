...
public class AnswerService implements Service {
	...
	public boolean hasDuplicateUS(int userid, int surveyid) {
		return jdao.queryForObject(" SELECT COUNT(*) FROM answer WHERE userid=" + userid + " AND surveyid=" + surveyid, Integer.class) > 0 ? true : false;
	}
			...
			public Integer doInTransaction(TransactionStatus status) {
				try {
					...
					for (Answer ans : answers) {
						...
						String sqlString = "INSERT INTO　answer(userid,number,type,surveyid,answertime,content" + opName + ") VALUES(?, ?, ?, ?, CONVERT(varchar(20),GETDATE() ,120) , ? " + opValue
								+ ")";
	 ...
	 * 返回分页信息：
	 */
	Map<String, Object> retPagination(String query_name, String date_from, String date_to, String pageSize, String pageNumber, String sort, String order) {
		...
		if (query_name == null && date_from == null && date_to == null) {
			...
			String tb_odr = "(SELECT ROW_NUMBER() OVER (ORDER BY " + sort + " " + order + ") AS odr ,tb_original.* FROM " + tb_original + ") AS tb_odr";
			...
			String tb_original = "(SELECT [user].name username,survey.title surveytitle,answer.* FROM ([user] INNER JOIN answer ON [user].id=answer.userid)INNER JOIN survey ON answer.surveyid= survey.id  "
					+ conditionString + ") AS tb_original";
	 ...
	 * 获取指定ID的记录
	 */
	List<Map<String, Object>> getAnswerByUSId(int surveyid, int userid) {
	 ...
	 * 获取指定ID的问题和答案记录
	 */
	List<Map<String, Object>> getAnswerAndSurveyByUSId(int surveyid, int userid) {
	 ...
	 * 获取指定userID的记录
	 */
	List<Map<String, Object>> getAnswerByUserId(int userid) {
